...
log = logging.getLogger("curio.monitor.notify")
...
class Notifier:
    ...
    def _expand_shortcut(name: str) -> list[str]:
        ...
        if name == "email":
            url = os.environ.get("CURIO_NOTIFY_EMAIL_URL")
            return [url] if url else []
        if name == "slack":
            url = os.environ.get("CURIO_NOTIFY_SLACK_URL")
            return [url] if url else []
        if name == "discord":
            url = os.environ.get("CURIO_NOTIFY_DISCORD_URL")
            return [url] if url else []
        if name == "webhook":
            url = os.environ.get("CURIO_NOTIFY_WEBHOOK_URL")
            return [url] if url else []
        return [name]
