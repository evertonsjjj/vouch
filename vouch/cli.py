...
app = typer.Typer(
    ...
    help="vouch — curated AI search for agents.",
    no_args_is_help=True,
)
catalog_app = typer.Typer(help="Manage the site catalog.", no_args_is_help=True)
mcp_app = typer.Typer(help="Run vouch as an MCP server.", no_args_is_help=True)
...
def _main(
    ...
    if version:
        console.print(f"vouch {__version__}")
        raise typer.Exit()
...
def install_browser(
    ...
    try:
        ...
        console.print("[red]playwright is not installed. Run: pip install 'vouch[browser]'[/red]")
...
def serve(
    ...
    try:
        ...
        console.print("[red]Dashboard requires extras: pip install 'vouch[server]'[/red]")
        raise typer.Exit(1) from None
...
def catalog_add(
    ...
    db: Path = typer.Option(Path("~/.vouch/catalog.db").expanduser(), "--db"),
):
...
def mcp_serve(
    ...
    """Run vouch as an MCP server. Requires `pip install 'vouch[mcp]'`."""
    try:
...
def profiles_list():
    """List all curated site profiles bundled with vouch."""
...
def profiles_update(
    source: str = typer.Option(
        None, "--source", "-s", help="Override the registry URL (env: VOUCH_PROFILES_URL)"
    ),
...
def plugins_list():
    ...
    if not any((adapters, routers, profiles)):
        console.print(
            ...
            "PyPI for 'vouch-adapter-*', 'vouch-router-*', or "
            "'vouch-profiles-*' packages.[/dim]"
        )
        return
    ...
    _block("Adapters (vouch.adapters)", adapters)
    _block("Routers (vouch.routers)", routers)
    _block("Profile registries (vouch.profiles)", profiles)
