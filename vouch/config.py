...
class EngineConfig(BaseModel):
    ...
    # Caching --------------------------------------------------------------
    cache_dir: Path = Field(default_factory=lambda: _expand("~/.vouch"))
    ...
    use_browser_pool: bool = True  # share one Chromium across searches (engine lifetime)
    ...
    def _expand_cache(cls, v):
        return _expand(v) if v else _expand("~/.vouch")
