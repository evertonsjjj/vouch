...
try:
    ...
    raise ImportError("Dashboard requires extras: pip install 'vouch[server]'") from e
...
def build_app(*, catalog_path: Path | None = None) -> FastAPI:
    ...
    cache_dir = Path("~/.vouch").expanduser()
    ...
    app = FastAPI(title="vouch dashboard", version="0.1.0")
