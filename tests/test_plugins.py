...
def test_find_exact_match_wins():
    ...
    with patch("vouch.plugins._load", return_value=eps):
...
def test_plugin_failure_does_not_block_build_adapter():
    ...
    from vouch.adapters import build_adapter
    from vouch.config import EngineConfig
    ...
    with patch("vouch.plugins._load", return_value=[_BrokenEP()]):
