...
The community registry lives at::
    ...
    https://raw.githubusercontent.com/yourhandle/farol-profiles/main/profiles.yaml

(Or whatever URL is configured via ``FAROL_PROFILES_URL`` / the
``--source`` flag to ``farol profiles update``.)

The fetched YAML is saved to ``~/.farol/profiles_remote.yaml`` and merged
...
log = logging.getLogger("farol.profiles.update")
...
CACHE_FILE = (
    Path(os.environ.get("FAROL_HOME", str(Path.home() / ".farol"))) / "profiles_remote.yaml"
)
...
def update_profiles(
    *,
    ...
    headers = {"User-Agent": "farol-profiles/1.0"}
