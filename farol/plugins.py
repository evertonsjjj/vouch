...
farol picks up three plugin groups installed in the user's Python environment:

* ``farol.adapters`` — per-site or per-pattern adapters. Each entry's value
  is a callable returning an instance that implements :class:`SiteAdapter`
  (or a class that can be instantiated as such).
* ``farol.routers`` — alternative :class:`Router` strategies.
* ``farol.profiles`` — community-published profile registries. Each entry
...
--------------------
A third-party plugin ``farol-adapter-arxiv`` declares in its pyproject.toml::

    [project.entry-points."farol.adapters"]
    "arxiv.org" = "farol_adapter_arxiv:make"
...
That's it — ``pip install farol-adapter-arxiv`` and the next ``SearchEngine``
...
log = logging.getLogger("farol.plugins")
...
def find_adapter_factory(host: str) -> Any | None:
    ...
    for ep in _load("farol.adapters"):
        name = ep.name.lower()
        if name == host:
            exact = ep
            break
...
def list_adapter_plugins() -> list[str]:
    ...
    return sorted(ep.name for ep in _load("farol.adapters"))
...
def list_router_plugins() -> list[str]:
    return sorted(ep.name for ep in _load("farol.routers"))


def find_router_factory(name: str) -> Any | None:
    for ep in _load("farol.routers"):
        if ep.name == name:
            try:
                return ep.load()
            except Exception as e:
...
def list_profile_plugins() -> list[str]:
    return sorted(ep.name for ep in _load("farol.profiles"))


def load_profile_plugins() -> list[Any]:
    """Load every ``farol.profiles`` plugin and return their registries.

    Each entry point should be a callable returning a ``ProfileRegistry``.
    """
    out: list[Any] = []
    for ep in _load("farol.profiles"):
        try:
            factory = ep.load()
