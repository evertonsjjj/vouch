...
try:
    ...
    raise ImportError("Dashboard requires extras: pip install 'farol[server]'") from e
...
def build_app(*, catalog_path: Path | None = None) -> FastAPI:
    ...
    cache_dir = Path("~/.farol").expanduser()
    ...
    app = FastAPI(title="farol dashboard", version="0.1.0")
